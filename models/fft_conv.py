...
def fft_conv_magnitude(
    ...
    the convolution (in order ot mimic the PyTorch direct convolution).
    Args:
        magnitude: (Tensor) Input tensor to be convolved with the kernel.
        kernel: (Tensor) Convolution kernel.
        bias: (Tensor) Bias tensor to add to the output.
        padding: (Union[int, Iterable[int]) Number of zero samples to pad the
            input on the last dimension.
    ...
    """
    # Cast padding & stride to tuples.
    padding_ = to_ntuple(padding, n=magnitude.ndim - 2)
    stride_ = to_ntuple(stride, n=magnitude.ndim - 2)
    ...
    # Because PyTorch computes a *one-sided* FFT, we need the final dimension to
    # have *even* length.  Just pad with one more zero if the final dimension is odd.
    # if magnitude.size(-1) % 2 != 0:
    #     magnitude_ = f.pad(magnitude, [0, 1])
    # else:
    #     magnitude_ = magnitude

    kernel_padding = [
        pad
        for i in reversed(range(2, magnitude_.ndim))
        for pad in [0, magnitude_.size(i) - kernel.size(i)]
    ]
    padded_kernel = f.pad(kernel, kernel_padding)
    ...
    # signal_ = signal_.reshape(signal_.size(0), groups, -1, *signal_.shape[2:])
    # signal_fr = rfftn(signal_, dim=tuple(range(2, signal.ndim)))
    kernel_fr = rfftn(padded_kernel, dim=tuple(range(2, magnitude.ndim)))
...
def example_fft_conv():
    ...
    # Functional execution.  (Easiest for generic use cases.) fft_conv
